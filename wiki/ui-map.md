...
| Appzoom `- 0 +` | Sidebar topp | Alla inloggade | Zoomar hela appytan ut/in eller aterstaller till 100%. Samma funktion finns pa `Ctrl+-`, `Ctrl++`, `Ctrl+0` och `Ctrl+scroll`, och sparas lokalt i `flow-app-zoom`. | Om tabeller eller text kanns for stora/sma kan anvandaren justera utan webblasarmenyn. |
...
| Logg | Sidebar footer | Alla inloggade | Oppnar sidopanel med app-logg for t.ex. observations-uppdatering. Ikonen visar en badge med antal nya loggrader sedan panelen senast oppnades. | Tom logg betyder bara att inget har loggats i aktuell session. Badgen nollas nar loggen oppnas eller rensas. |
