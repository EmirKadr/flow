...
Matrisen sparas i appsettings som `allocation_process_matrix` via `GET/PUT /api/allokering/process-matrix`. Ytkartans redigerbara UTL-ytor sparas pa samma globala satt som `ytgenerering_map_layout` via `GET/PUT /api/allokering/ytgenerering-map-layout`. Bada galler for bade webb och desktop eftersom desktop servar samma frontend och API. Standardmatrisen ar:
