...
API-kallor hamtas via `ExternalDataClient.fetch_all` (delad `fetch_all_rows`), samma vag som Hamta data. Om datakallan kapar svaret vid radtaket (`DATA_SOURCE_RESPONSE_ROW_CAP`) delas hamtningen upp i datumfonster pa ett Between-datumfilter och slas ihop, sa stora underlag (t.ex. helar) inte tyst trunkeras. Tidigare anvande Bearbeta/Produktivitet `fetch_data` direkt utan uppdelning.
...
Ytor utan placering i resultatkartans SVG anvander samma dynamiska textstorlek som placerade kundnamn, och staende tomma ytor roterar ytkoden langs ytan. Det gor att lediga ytor syns tydligt aven nar kartan ar utzoomad.
