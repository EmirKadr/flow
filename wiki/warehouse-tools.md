...
| Redigera kluster | Visas i Forecast-resultatet nar Forecast har transportorer. Oppnar en modal byggd som referensdemons *Advanced settings*-tabell: drag-sortering (radordningen satter `assignment_order`), index, Transportor och kolumnerna ASN/Arrive/Depart, Group (`cluster_group`), Start seq/End seq och en fargvaljare. ASN/Arrive/Depart seedas med demons standardtider (11:00/12:00/14:00) och sparas i `carrier_clusters` som metadata (styr inte placeringen). Om `trans_agency` inte gav klusterrader skapas raderna fran Forecast-tabellens unika transportorer. |
