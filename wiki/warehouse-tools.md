...
Uppladdningar visar separata listor for permanenta karnfiler och sammanstalld data. Vanliga filrader visar alltid det svenska vy-/slotnamnet fran filkunskapen som fet rubrik, sa tekniska alias som `customer_order_details_all` visas som `Detalj kundorder(alla)`. `artikel_max.csv` ar sammanstalld data och uppdaterar samma verksamhetsfil som Ordersaldo, LYX och Pafyllnadsprio anvander. Produktivitetens tre sammanstallda loggfiler visas ocksa har: `productivity_pick_observations`, `productivity_trans_observations` och `productivity_pallet_observations`. Coredata-prefixen `custom`, `dimension`, `dispatch_template`, `item`, `item_alias`, `item_attribute`, `item_option`, `item_security_info`, `kpi_target_rule`, `location`, `location_cost`, `pallet_type` och `trans_agency` sparas som blobbar i Postgres-tabellen `coredata_files` med unik nyckel per verksamhet och filtyp. `trans_agency` ar transportors-/agency-karnfilen och kan aven laddas upp med filnamn som borjar pa `transportorer`, `transportor` eller `agency`. Om en anvandare laddar upp en ny fil med samma prefix for sin verksamhet ersatts DB-raden och den nya blir sanningen for alla anvandare i verksamheten. Andra verksamheters filer rors inte. Gamla filer under `data/coredata/<verksamhetskod>/` kan fortfarande lasas som fallback tills de laddas upp igen.
...
Dolda/tekniska floden finns for observations-update, observations-sync och update-check. Observations kan aven triggas automatiskt nar ny buffertfil laggs in. Vid serverstart kors en bakgrundssync for observations, men den vantar forst `ALLOCATION_OBSERVATIONS_STARTUP_DELAY_SECONDS` och kor sedan alla aktiva verksamheter fran databasen en i taget med `ALLOCATION_OBSERVATIONS_STARTUP_SPACING_SECONDS` mellan varje. Det minskar Render-cold-start-spikar eftersom legacy-lagerverktyget, pandas och observationsfilen inte laddas direkt nar webbtjansten startar. Syncen kan stangas av med `ALLOCATION_OBSERVATIONS_STARTUP_SYNC=false` om den ska ersattas av manuellt eller schemalagt flode. Observations och den framraknade sammanstallda datan `artikel_max.csv` ar verksamhetsseparerade med samma undermappsregel for alla verksamheter: `lowfreqdata/buffertpall/stigamo/`, `lowfreqdata/buffertpall/r3/`, `lowfreqdata/buffertpall/t3/` och sa vidare. Gamla root-filer kan endast anvandas som seed/legacy-ingang for Stigamos nya undermapp; aktiva las- och skrivvagar gar via verksamhetsmappen.
