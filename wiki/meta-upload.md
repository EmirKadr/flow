...
| Sandningsanalys | `meta.html` | Super User | Visar ASK-berikade lookup-kolumner for ordernummer, sandningsnummer, anvandarnamn och kund, samt rosttolkat pall-id, avvikelser, status, uppdaterad timestamp, video, videolangd, videostorlek, stillbild och hash | `GET /api/meta/shipment-observations` | Tom eller `LLM saknas` om Gemini inte ar konfigurerad. Om Dispatchpallar saknar traff visas raden med osakerhetsanteckning. |
| Analysera | `meta.html` | Super User | Extraherar ljud fran videon och skickar bara rosten till Gemini for pall-id och avvikelser. | `POST /api/meta/uploads/{upload_id}/analyze` | Kraver `GEMINI_API_KEY`. Osakra svar hamnar i `Kontrollera`. Ovantade fel (t.ex. ffmpeg-timeout vid etikettstillbild) markerar raden som `Fel` med felmeddelandet synligt och full traceback i loggen — de lamnar inte langre raden last i `Analyserar` med en naken 500. |
...
- Ny media far status `pending_analysis`. For videor skapas `meta_shipment_observations` med `video_hash` och `record_hash`. Meta-vyn visar videons filnamn, kort video-hash, videolangd, videostorlek, stillbildsstatus och rad-id direkt i sandningsanalysen sa Super User kan se vilken rad som tillhor vilken video och vilka filmer som ar tunga att analysera.
