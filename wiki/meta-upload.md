...
| "Varfor fick jag 503 och sidan dog nar jag klickade Analysera?" | 2026-07-09 dog poddprocessen ~1 min efter ett Analysera-klick (raden fastnade i `Analyserar`, alla bakgrundsjobb fick ny starttid). Trolig orsak ar resurstryck i podden (drift kor CPU-limit 300m och Octopus-styrd minneslimit; probarnas default-timeout var 1 s). Probarna har nu `timeoutSeconds: 5` i bade `k8s/flow.yml` och `k8s/deployment.yaml`. Vid aterfall: ta `kubectl describe pod` och kolla `lastState` (OOMKilled?) samt events. |
