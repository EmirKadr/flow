...
| Ladda ner | `meta.html` | Super User | Koar en browser-nedladdning av videon fran sandningstabellens ikonknapp | content-URL fran `GET /api/meta/shipment-observations`, `api.download(...)` | 403 om anvandaren inte ar Super User. 404 om lagrad media saknas. Playable-video transkodas en i taget pa servern. |
| Sandningsanalys | `meta.html` | Super User | Visar numeriskt Analys-ID, ASK-berikade lookup-kolumner for ordernummer, sandningsnummer, anvandarnamn och kund, samt rosttolkat pall-id, avvikelser, status, uppdaterad timestamp, video, videolangd, videostorlek och radhash | `GET /api/meta/shipment-observations` | Analys-ID ar observationens databas-id och visas som `#<id>`. Det ar inte samma varde som uppladdnings-id eller radhash. Tom eller `LLM saknas` om Gemini inte ar konfigurerad. Om Dispatchpallar saknar traff visas raden med osakerhetsanteckning. |
| Analysera | `meta.html` | Super User | Extraherar ljud fran videon och skickar bara rosten till Gemini for pall-id och avvikelser. | `POST /api/meta/uploads/{upload_id}/analyze` | Kraver `GEMINI_API_KEY`. Osakra svar hamnar i `Kontrollera`. Ovantade fel markerar raden som `Fel` med felmeddelandet synligt (sanerat fran nycklar) och full traceback i loggen — de lamnar inte raden last i `Analyserar` med en naken 500. ASK-uppslag som felar eller saknar traff blir en osakerhetsanteckning med pall-id, aldrig ett fel. |
...
- Meta-vyn laddar ner videon fran sandningsanalysens ikonknapp via en enkel nedladdningsko (`META_DOWNLOAD_CONCURRENCY=1`). Lanken kommer fran sandningsanalysens content-URL med `download=1`. Playable-video (`variant=playable`) transkodas en i taget pa servern via en backend-semafor, med `-threads 2` sa transkodningen inte kan OOM-doda podden.
...
- API-nyckeln skickas i `x-goog-api-key`-headern, aldrig som `?key=` i URL:en (URL:er hamnar i undantagstexter och loggar). Sparade analysfel (`analysis_error`) saneras dessutom fran nyckeln innan de skrivs till DB/UI. Settings blankar varden som ar osubstituerade Octopus-platshallare (`#{VAR}`), och `_gemini_base_url()` faller tillbaka pa Googles standard-URL for allt som inte ar en http(s)-URL. Bakgrund: incident 2026-07-09, se felsokningsraden nedan.
...
- Efter en lyckad rostanalys forsoker backend hamta Dispatchpallar fran ASK via den konfigurerade externa datakallan. Uppslaget anvander `v_ask_dispatch_pallet` med filter pa `pick_pall_num` och fyller `order_num`, `shipment_id`, `user_id` och `custom_desc` till Meta-radens ordernummer, sandningsnummer, anvandarnamn och kund. Ger live-vyn ingen traff (den haller bara ~14 dagar, se `ask-datalagring.md`) gors samma uppslag mot arkivet `dblog_dispatch_pallet_log` (~800 dagar); arkivet saknar `custom_desc` sa kundfaltet blir da kundnumret (`custom_num`). Numeriska pall-id skickas som tal sa EQ-filtret matchar den numeriska kolumnen. Alla utfall utan traff (ej konfigurerad kalla, API-fel, ovantat fel, ingen traff ens i arkivet) blir en osakerhetsanteckning med pall-id — aldrig ett analysfel.
...
- `GET /api/meta/uploads`, `GET /api/meta/uploads/{upload_id}/content`, `GET /api/meta/shipment-observations`, `GET /api/meta/shipment-observations/export`, `POST /api/meta/uploads/{upload_id}/analyze`, `PATCH /api/meta/shipment-observations/{observation_id}/status` och `DELETE /api/meta/uploads/{upload_id}` kraver Super User. `meta.html` anvander `initPage("meta", { requireSuperUser: true })` och visas bara for Super User i sidebaren.
- Manuellt statusbyte gar via `PATCH /api/meta/shipment-observations/{observation_id}/status` med body `{"status": "..."}`. Tillatna varden ligger i `EDITABLE_SHIPMENT_STATUSES` i `meta_uploads.py` och maste hallas i synk med `SHIPMENT_STATUS_OPTIONS`/`SHIPMENT_STATUS_LABELS` i `meta.js`. Bytet audit-loggas som `entity_type=meta_shipment_observation`, `action=update_status` med gammalt och nytt `analysis_status`.
...
| "Kan jag kora analysen fran min egen dator utan att oppna UI:t?" | Ja: `python -m tools.flow_cli --base-url <miljo> auth login` foljt av `python -m tools.flow_cli --base-url <miljo> meta process-queue`. `auth login` tar `--username`/`--password`, men faller annars tillbaka pa `FLOW_E2E_USERNAME`/`FLOW_E2E_PASSWORD` (miljovariabel eller `.env`/`app/.env`, samma variabler som e2e-verktyget), sa inloggning kan koras utan flaggor om de redan ar satta. Kommandot hamtar sandningsrader med given status (default `queued`) och anropar `/analyze` for varje, exakt samma serverflode som `Analysera`-knappen (ffmpeg-ljudextraktion, Gemini, ASK-uppslag, statusbyte sker allt server-side). Skiljer sig fran `tools/meta_analysis_worker.py` genom att den pratar HTTP mot en driftmiljo istallet for att kravas direkt DB-atkomst, sa den funkar fran vilken dator som helst med natverksatkomst. `--status` valjer vilken analysstatus som plockas, `--timeout` hojer las-timeouten (default 600s) om langa videor annars hinner timea ut klient-side. |
...
| "Varfor fick jag 502/503 och sidan dog nar jag klickade Analysera?" | Incident 2026-07-09, bekraftad orsak via Seq-trace + lokal minnesmatning: ffmpeg kor som default tradar for alla karnor, och multitradad h264-avkodning av Meta-glasogonvideo (1488x1984@120fps) toppade ~254 MB. Ovanpa appens ~150-190 MB passerade cgroupen minneslimiten och kubelet grupp-OOM-dodade hela containern (cgroup v2 dodar alla processer, inte bara ffmpeg) — mitt i etikettstillbilds-steget, EFTER att Gemini-analysen lyckats. Symptom: 502/503 i UI, raden fast i `Analyserar`, inget i Seq (SIGKILL loggar inte), bakgrundsjobben far ny starttid. Fix: `-threads 1` pa analysens ffmpeg-avkodningar (~60 MB) och `-threads 2` pa playable-transkodningen (avkodare + x264); kontraktstest lasar flaggorna. Probarna har ocksa `timeoutSeconds: 5`. Vid aterfall: `kubectl describe pod`, kolla `lastState=OOMKilled`, och overvag hojd `JOB_MEMORY_MAX` i Octopus. |
