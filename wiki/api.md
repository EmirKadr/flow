...
- `GET /api/sankey/inbound` - separat Sankey - Inbound-rapport, kräver `sankeyInbound=view`. Query `period=day|week|month|year`, `date`, valfri `company` och `only_consumed` väljer mottagningskohort och om öppna etiketter ska visas. Backend följer etiketter från mottagning fram till idag via receive/trans/pick/loading live- och `dblog_*`-vyer. Gross income kommer från `inbound_labels` per mottagen etikett plus `inbound_article_rows` per unik `book_num + line_num`; båda intäktsdelarna följer samma branch och processfördelning. Svaret returnerar `nodes`, `links`, `processes`, `summary`, `companies`, `warnings` och `source_status`, inklusive `gross_income_labels`, `gross_income_purchase_lines`, `label_revenue` och `purchase_line_revenue`. Körningar auditloggas som `sankey_inbound_report/run|run_failed` utan pallid/order/radpayload.
