...
- `GET /api/meta/uploads`, `HEAD/GET /api/meta/uploads/{upload_id}/content`, `GET /api/meta/uploads/{upload_id}/audio`, `DELETE /api/meta/uploads/{upload_id}` - Super User-endpoints for Meta-vyn. Content-endpointen strommar originalvideon och audio-endpointen extraherar en separat MP3. Video- och ljudnedladdning audit-loggas utan filnamn eller innehall. `variant=playable` ar fortsatt alias till originalfilen. Delete-endpointen raderar raden och blobben.
...
- `PATCH /api/meta/shipment-observations/{observation_id}/local-analysis` - tar emot sanerade resultatfalt fran `flow_cli --local-analysis`. Full transkription och media far inte skickas eller auditloggas.
