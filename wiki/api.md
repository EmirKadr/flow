...
- `GET /api/sankey/inbound` - separat Sankey - Inbound-rapport, kräver `sankeyInbound=view`. Query `period=day|week|month|year`, `date`, valfri `company` och `only_consumed` väljer mottagningskohort och om öppna etiketter ska visas. Backend följer etiketter från mottagning fram till idag via receive/trans/pick live- och `dblog_*`-vyer samt aktuell buffertpallstatus. Gross income kommer från `inbound_labels` per mottagen etikett plus `inbound_article_rows` per unik `book_num + line_num`; båda intäktsdelarna följer samma branch och processfördelning. Svaret returnerar `nodes`, `links`, `processes`, `trace_rows`, `summary`, `companies`, `warnings` och `source_status`, inklusive `gross_income_labels`, `gross_income_purchase_lines`, `label_revenue`, `purchase_line_revenue` samt spårningsrader med pallid, inköp/rad, `received_date` och stegvis väg för klick/export i UI. Standardpayloaden innehåller även `client_filters.only_consumed`, direkt `only_consumed=true` innehåller `client_filters.all`, och nyare svar innehåller `client_filters.views` för lokala bolags-, dag- och månadsväxlingar utan ny hämtning när urvalet ryms i redan hämtad period. Körningar auditloggas som `sankey_inbound_report/run|run_failed` utan pallid/order/radpayload.
