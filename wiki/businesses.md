...
- Gemensamma coredata-karnfiler ar verksamhetsseparerade i Postgres-tabellen `coredata_files`. Varje verksamhet har en egen rad per filtyp, identifierad med `business_code + file_type`. En ny uppladdning ersatter bara gamla DB-raden med samma prefix i anvandarens egen verksamhet. Gamla filer under `data/coredata/` kan fortfarande lasas som fallback tills de laddas upp igen.
